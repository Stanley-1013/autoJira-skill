...
MR_TEMPLATE = """## Jira
- **Issue**: [{key}]({jira_base_url}/browse/{key})
{epic_line}
...
def generate_mr_description(
    ...
    epic_line = ""
    if epic_key:
        epic_line = f"- **Epic**: [{epic_key}]({jira_base_url}/browse/{epic_key})"

    return MR_TEMPLATE.format(key=jira_key, epic_line=epic_line, jira_base_url=jira_base_url)
...
def main():
    ...
    if args.command == "validate":
        ...
        desc = generate_mr_description(args.jira_key, args.epic, jira_base_url=args.jira_base_url)
        print(desc)
