...
def pack_issue(
    ...
    *,
    ...
    if isinstance(desc, dict):
        ...
        desc = desc.strip()
        out.append(truncate_text(desc, max_desc))

    # Comments (if present)
    comments_list = get_path(raw, "fields.comment.comments", [])
    if comments_list and comments > 0:
        out.append("")
        out.append("## Recent Comments")
        # Show last N comments only
        for comment in comments_list[-comments:]:
            ...
            body = truncate_text(str(body), max_comment)
            out.append(f"\n**{author}** ({created}):")
            out.append(body)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Pack a Jira issue JSON into compact markdown (token-efficient)"
    )
    parser.add_argument(
        "file",
        nargs="?",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4000,
        help="Max characters for Description section (default: 4000)",
    )
    parser.add_argument(
        "--comments",
        type=int,
        default=3,
        help="How many recent comments to include (default: 3; 0 disables)",
    )
    parser.add_argument(
        "--max-comment",
        type=int,
        default=500,
        help="Max characters per comment body (default: 500)",
    )
